...
class ChargePoint(cp):
    ...
    def load_config(self):
        try:
            ...
            self.connectors_status = {i: ConnectorStatus.available for i in range(1, self.config["NumberOfConnectors"] + 1)}
    ...
    async def start_transaction(self, connector_id, id_tag):
        if self.connectors_status[connector_id] == ConnectorStatus.available:
            ...
            if response.id_tag_info.status == AuthorizationStatus.accepted:
                ...
                self.connectors_status[connector_id] = ConnectorStatus.charging
...
async def main():
    ...
    try:
        ...
        async with websockets.connect(
            ...
            cp_instance = ChargePoint("CP_1", ws, config_file, pzem_ports, relay_pins)
