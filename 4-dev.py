...
class ChargePoint(cp):
    ...
    async def start_transaction(self, connector_id, id_tag):
        if connector_id not in self.active_transactions:
            ...
            if not authorize_response:
                print(f"Authorization failed for idTag {id_tag}. Transaction not stopped.")
                return False
    ...
    async def stop_transaction(self, connector_id):
        if connector_id in self.active_transactions:
            ...
            meter_stop = self.get_meter_value(connector_id)['power'] * 1000
            ...
            try:
                ...
                print(f"Error occurred while stopping transaction: {e}")
    ...
    async def on_remote_start_transaction(self, **kwargs):
        ...
        # Start transaction
        transaction_started = await self.start_transaction(connector_id, id_tag)
